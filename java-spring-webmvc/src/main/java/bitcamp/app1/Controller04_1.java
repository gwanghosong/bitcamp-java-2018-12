// request handler(요청 핸들러)의 아규먼트 - 프론트 컨트롤러로부터 받을 수 있는 파라미터 값
...
public class Controller04_1 {
  ...
  // 그러면 프론트 컨트롤러가 메서드를 호출할 때 해당 타입의 값을 넘겨 줄 것이다.
  
  // ServletContext는 의존 객체로 주입 받아야 한다.
  // 요청 핸들러에서 아규먼트로 받을 수 없다.
  @Autowired ServletContext sc;
  
  // 요청 핸들러에서 받을 수 있는 아규먼트를 선언해보자.
  @GetMapping("h1")
  @ResponseBody
  public void handler1(
      ...
      Model model, // map과 같다. 둘 중 한개만 받으면 된다.
      PrintWriter out // 클라이언트에게 콘텐트를 보낼 때 사용할 출력 스트림
      ) {
    
    // ServletRequest와 ServletResponse,
    // HttpServletRequest와 HttpServletResponse, HttpSession
    // Map<String,Object>, Model, PrintWriter
    // 객체를 받을 수 있다.
