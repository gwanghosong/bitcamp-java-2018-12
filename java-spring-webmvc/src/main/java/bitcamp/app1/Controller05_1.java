// request handler(요청 핸들러)의 리턴 값 - 콘텐트를 직접 리턴하기
...
public class Controller05_1 {
  ...
  public String handler1() {
    ...
    //    또한 한글은 ISO-8859-1 문자표에 정의된 코드가 아니기 때문에 
    //    클라이언트로 보낼 때 '?' 문자로 바꿔보낸다.
  ...
  public ResponseEntity<String> handler7(HttpServletResponse response) {
    // HttpEntity 대신에 ResponseEntity 객체를 리턴할 수 있다.
    ...
    ResponseEntity<String> entity = new ResponseEntity<>(
        "<html><body><h1>abc가각간</h1></body></html>",
        headers,
        HttpStatus.OK // 응답 status 코드를 지정할 수 있다. 200 OK
        );
