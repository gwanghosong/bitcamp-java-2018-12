...
// : 그리고 그 서블릿 인스턴스는 클래스마다 오직 한 개만 생성된다.
// : init(), destroy()는 오직 한 번만 호출된다.
// : service()는 클라이언트가 요청할 때마다 호출된다.
//
// 주의!
// : 서블릿 인스턴스는 오직 클래스마다 한 개만 생성된다.
// : 그래서 모든 클라이언트가 같은 서블릿 인스턴스를 사용한다.
// : 클라이언트마다 구분되어야 할 데이터는 인스턴스 변수에 보관해서는 안된다.
...
public class Servlet01 implements Servlet { 
  ...
  public void init(ServletConfig config) throws ServletException {
    ...
    this.config = config;
    System.out.println("Servlet01.init(ServlentConfig)");
  }
  ...
  public String getServletInfo() {
    // 서블릿 컨테이너의 관리자 화면을 출력할 때 이 메서드를 호출한다.
