...
public class DispatcherServlet extends HttpServlet {
  ...
  protected void service(
    ...
    try {
      ...
      //  : requestHandler(요청이 들어왔을 때 호출되는 메서드) 실행
      String viewUrl = 
          (String) requestHandler.method.invoke(
    ...
    for (Parameter param : params) {
      ...
      if (paramType == ServletRequest.class ||
        ...
        paramValues.add(request);
          ...
          paramType == ServletResponse.class ||
    ...
    try {
      ...
      for (Method method : methods) {
        ...
        String propName = method.getName().substring(3, 4).toLowerCase() +
            method.getName().substring(4); // 배열 0부터 시작

        // 프로퍼티 이름으로 요청 파라미터 값을 찾아서 세터를 호출한다.
        //  : 메서드의 파라미터 타입을 알아낸다.
        Class<?> type = method.getParameterTypes()[0]; // 세터의 파라미터는 오직 1개이다.
  ...
  private Object getParameterValue(
    ...
    if (methodParamType == int.class) {
      ...
      ArrayList<Part> list = new ArrayList<>(); // Part 객체를 담을 배열 준비 
      for (Part part : parts) {
