...
// 있다면 그 세터를 호출하여 의존객체를 주입한다.
// 의존 객체가 없다면 그 의존 객체가 생성될 때까지 
// 별도로 담아 둔다.
// 의존 객체가 생성되는 순간 즉시 별도로 담아 둔 그 객체에 대해 세터를 호출할 것이다.
//
public class MyAutowiredAnnotationBeanPostProcessor implements BeanPostProcessor {
  ...
  public Object postProcessAfterInitialization(Object bean, String beanName) 
    ...
    for (Method m : methods) {
      ...
      if (anno == null) // @Autowired가 붙지 않은 메서드는 무시한다.
        continue;
      // 애노테이션이 있다면 이 세터 메서드가 어떤 타입의 값을 원하는지 알아낸다.
      Class<?> paramType = m.getParameters()[0].getType();

      // 세터가 원하는 값이 있는지 확인한다.
      if (isRegistered(paramType)) {
        // 세터가 원하는 파라미터 값이 있다면 즉시 호출하여 의존객체를 주입한다.
        try {
  ...
  private void addAutowiredMethod(
    ...
    // 이 파라미터가 필요한데 없어서 저장해 놓은 Map에서 이 파라미터가 필요한 
    // 모든 @Autowired 애노테이션이 붙은 세터메서드와 그 메서드를 실행하는데 필요한
    // 객체로 이루어진 클래스인스턴스 AutowiredMethod 배열객체들을 꺼낸다.
    List<AutowiredMethod> methods = autowiredMethodMap.get(paramType);
    if (methods == null) {// 파라미터 타입의 값을 원하는 세터가 등록된 적이없다면,
  ...
  class AutowiredMethod {
    Object object; // 메서드가 호출할 때 사용할 인스턴스
