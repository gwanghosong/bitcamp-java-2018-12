  ...
  public static void main(String[] args) {
    try (Scanner keyboard = new Scanner(System.in);
      ...
      try(
        ...
        System.out.println("클라이언트와 통신할 입출력 스트림이 준비 되었음.");

        // Client1과 Server1의 통신 규칙에 따라 순서대로 입출력 해야 한다.
        // 왜? 입출력은 blocking 모드로 작동하기 때문이다.
        // 클라이언트와 서버간의 데이터를 주고 받는 통신 규칙을 "프로토콜(protocol)"이라 한다.
        // 에코(echo)는 클라이언트에서 한 줄의 문자열을 보낸다.
        // 따라서 서버는 한 줄의 문자열을 읽은 후에 응답해야 한다.

        // 서버에서 한 줄의 데이터를 읽기 전에는 클라이언트의 write()이 리턴하지 않음을
        ...
        // 서버가 데이터를 보내지 않으면 클라이언트에 read()은 리턴하지 않는다.
