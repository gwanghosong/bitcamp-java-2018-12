// 기능 확장 - 방법1) 기존 코드를 가져와서 새 기능을 추가하기(복붙하기)
package ch13.a;
...
1) 기존 클래스 파일에 추가하는 방법
 ...
 - 무슨말??? 다른 개발자가 배포한 라이브러리만 있는 경우를 말한다
 - 즉, 소스가 없는 다른 개발자가 만든 클래스에 기능을 덧 붙일 때는 이 방법이 불가능하다.
 - 기존 코드에 버그가 있을 때 복사붙여넣기 해서 만든 클래스도 영향을 받는다.
 - 기존 코드를 변경했을 때 복사 붙여넣기한 모든 클래스를 찾아 변경해야 한다.
