...
// Convertible이나 DumpTruck을 만든 것이 아니라,
// 여러 클래스를 사용하다 보니 공통분모가 보여서,
// 소스 코드의 유지보수가 쉽도록 한 클래스로 모아두기 위해 만든 경우는 
// 해당 클래스를 직접 사용할 이유가 없다.
// 특히 generalization을 수행하여 만든 수퍼 클래스의 경우
// 직접 사용할 목적으로 만든 클래스가 아니다.
// 다만 서브 클래스에 공통 기능을 물려주기 위해 존재하는 클래스이다.
// 이런 클래스들은 직접 사용하지 못하게 막는 것이 좋다.
// 일반적으로 generalization으로 만든 수퍼 클래스는
// 추상클래스로 만든다.

// 클래스를 직접 사용하지 못하게 막고 단지 서브 클래스를 만들어 사용하도록
// 제한하는 문법이 " 추상 클래스"이다.
