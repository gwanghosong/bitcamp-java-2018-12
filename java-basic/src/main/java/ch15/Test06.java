...
public class Test06 {
  ...
  public static void main(String[] args) {
    ...
    // 언제든 다른 데이터에 대해 같은 값이 나올 가능성을 내포하고 있는 것이다.
    // 다만 확률이 얼마나 낮은가의 문제다.
    // 다른 데이터에 대해 같은 정수 값이 나올 확률이 낮을수록 안심하고 사용할 수 있다.
    // 현실과 비교하면 주민번호와 같다.
    // 본인 여부를 확인할 때 주민 번호로 확인하면 매우 빠르게 결과를 알 수 있다.
    // 원래는 본인인지 알아내려면 DNA검사를 해야 한다.
    // 그러면 시간이 너무 오래 걸린다. 그런데 지문이나 주민번호를 사용하면 빠르게 본인 여부를 알 수 있다.
    // 이런 이유로 hash 값을 "디지털 지문"이라 부른다.
    // 목적?
    // - 데이터가 같은지 빠르게 비교하게 위함.
    // 응용?
    // - 본인 여부를 확인하는 인증서.
    // - 파일의 위변조를 검사하는 용도. 
    //  예1) git에서 커밋할 때 고유번호를 붙이는데 바로 해시 값이다.
    //  예2) 파일 다운로드 사이트에서 제공하는 해시값
