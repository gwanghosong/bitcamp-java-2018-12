...
// clone() 인스턴스를 자동 복제 할 때 호출하는 메서드이다.
  
public class Test16 {
  
  // 인스턴스 복제 기능을 활성하려면 Cloneable 인터페이스를 구현해야 한다.
  // 이 인터페이스에는 메서드가 선언되어있지 않다. 
  // 따라서 클래스는 메서드를 따로 구현할 필요가 없다.
  // Cloneable을 구현하는 이유는 JVM에게 이 클래스의 인스턴스를 복제할 수 있음을 표시하기 위함이다.
  // 이 표시가 안된 클래스는 JVM이 인스턴스를 복제해주지않는다.

  // 인스턴스를 복제할 때는,
  // Object에서 상속받은 clone()을 호출해야 한다.
  // 그런데 clone()의 접근 제어가 protected이라서 
  ...
  static class Score implements Cloneable {
    ...
    // 오버라이딩 할 때 리턴 타입을 클래스 타입으로 변경해도된다.
    @Override
  ...
  public static void main(String[] args) throws Exception {
    ...
    //만약 Score에 java.lang.Cloneable인터페이스를 붙이지 않았다면,
    // JVM은 다음 예외를 발생시킨다.
