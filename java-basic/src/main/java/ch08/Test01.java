...
class My1 {
  ...
  // 인스턴스를 만들 때 마다 해당 변수를 매번 만드는 것은 메모리를 낭비하는 것이다.
  // 근데 이렇게 선언하면 클래스에 따른 인스턴스를 생성할때마다
  // manager, member, guest에 똑같은 값("관리자", "회원", "손님")
  //  을 메모리에 입력하며 만들어진다.
  // 즉 메모리 낭비가 생김!
  // 해법?
  // "클래스필드!" 문법
  // 인스턴스를 생성할 때 만드는 변수가 아니라 클래스를 로딩할 때 만드는 변수이다.
  // 클래스는 JVM마다 딱 한 번만 로딩되기 때문에 "클래스 필드"도 딱 한 번만 생성된다.
  // 그래서 메모리를 절약할 수 있다.
  // 클래스 필드를 만드는 문법
  //      static 데이터타입 필드명(변수명);
  // 클래스 필드 사용법
  // 인스턴스 필드를 사용하려면 
  //         레퍼런스(인스턴스주소).필드명 = 값;
  // 클래스 필드를 사용하려면
...
public class Test01 {
  public static void main(String[] args) {
    ...
    // 값을 직접 입력하면 "회운"처럼 입력하면, 컴파일은 가능, but 걸러낼 수가 없다.
    // 오타 가능성이 높다.

    // 오타가 나올 가능성이 현저하게 감소한다.
    // 근데 다른곳에서도 이걸 쓰고 싶다면?
    // 정확히 말하면 My1이라는 데이터 타입, 클래스에서 사용하고 싶다면?
    // 메인 메소드가 아니라 My1 클래스에서 선언하자!
  }
}
