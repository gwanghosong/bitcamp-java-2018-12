...
public class Test02 {
  public static void main(String[] args) {
    ...
    //     - My2 클래스가이미 로딩되었기 때문에 다시 로딩하지않는다.
...
/*
 클래스 로딩
 - 외부 저장장치(HDD, USB메모리, DVD-ROM)에 있는 .class 파일을
  JVM이 관리하는 메모리로 로딩하는것.
 ...
 3) Method Area
   ...
   - JVM은 코드를 실행할 때 이 영역에 놓인 명령어를 실행하는 것이다.
   ...
   - new 명령을 사용하여 인스턴스를 생성할 때
       new My2();
   - 주의!
   - 레퍼런스 변수를 선언할 때는 클래스를 로딩하지 않는다. 로딩하지 않는다! 로딩하지 않는다!
   My2 obj1;
   - 주소를 담는 변수이기 때문에 메모리가 담기지 않아서 구분할 필요가 없다.
   - 즉 여기엔 인스턴스 메모리에 주소가 담기기 때문에 클래스형태의 메모리를 만들지를 않는다.
   - 즉 클래스를 로딩할 필요가 없다.
   - 따라서 클래스 로딩하지 않는다.
   - 이러한 형태의 메모리를 만드는 명령은 new My2();이다.
   - 이 때 메모리에 클래스가 올라온다.
 */
