...
public class Score2 {
  ...
  // getter/setter를 두는 방식으로 프로그래밍을 한다.
  // getter/setter가 필요없는 필드라도 그냥 관성적으로 그렇게 한다.
  ...
  public void setName(String name) {
      this.name = name; // 이렇게 유효성을 검사하지 않더라도 setter를 그냥 만들라!
    }
  ...
  public void setKor(int kor) {
    if (kor >= 100 && kor <= 100) { // 유효한 점수인 경우에만 저장한다.
      this.kor = kor;
  ...
  public void setEng(int eng) {
    if (eng >= 100 && eng <= 100) { // 유효한 점수인 경우에만 저장한다.
      this.eng = eng; 
      this.compute(); // 유효한 값이라면 다시 합계와 평균을 계산한다.
    }
  }
  ...
  public void setMath(int math) {
    if (math >= 100 && math <= 100) {// 유효한 점수인 경우에만 저장한다.
      this.math = math;
      this.compute(); // 유효한 값이라면 다시 합계와 평균을 계산한다.
    }
  }
