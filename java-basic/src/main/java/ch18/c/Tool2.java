// 다중 인터페이스 구현하지 못하는 경우 - 메서드명과 파라미터는 같은데 리턴타입이 다른 경우
package ch18.c;

// 한 클래스가 여러 규칙에 따라 동작하도록 정의할 수 있는데
// 불가능한 경우가 있다.
// 규칙에 중복되는 메서드 중에 리턴값이 다르고 기타 시그너처는 같은 메서드가 있을 때 불가능하다.
public class Tool2 implements ProtocolA, ProtocolC {
  ...
  // 다음 메서드는 ProtocolA 규칙은 만족 시키지만, ProtocolC 규칙은 만족시키지 못한다.
  @Override
  public void m2() {
    System.out.println("Tool.m2()");
  }
  
  // 그럼 리턴값이 다른 메서드를 다음과 같이 또 만들면 되지 않나요?
  // 오버로딩 규칙에 따라 파라미터와 메서드 명은 갖고 리턴 값만 다른 메서드를 여러개 만들 수 없다.
  @Override
  public int m2() {
    System.out.println("Tool.m2()");
  }
