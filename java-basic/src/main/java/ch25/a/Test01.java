...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    // 2) 라이브러리 정보를 build.gradle에 설정한다.
    ...
    // DriverManager는 사용할 애플리케이션에 대해 
    //   사용 가능한 JDBC(Java Database Connectivity) 드라이버 세트를 관리합니다.
    
    // 1) java.sql.Driver 구현체를 생성하여 JDBC 드라이버 관리자에 등록한다.
    // Driver 구현체는 client machine에 설치된 JDBC 드라이버에 정보를 제공하는 클래스이다.
    ...
    // DriverManager는 이전에 등록된 Driver 구현체(new org.mariadb.jdbc.Driver())에게 위임한다.
    ...
    //                 포트번호를 지정하지 않으면 3306이 기본값으로 설정된다.
