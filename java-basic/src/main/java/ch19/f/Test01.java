...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    // 같은 식구가 된 Array는 LinkedList.this라는 
    // 이름으로 바깥 클래스의 인스턴스 주소를 사용할 수 있다.
    // 3) Array는 자기 것인양 LinkedList의 멤버를 사용하여 해당 값을 꺼내 배열을 만들어 리턴한다.
    // 4) LinkedList는 Array로부터 받은 배열을 그대로 리턴한다.
    // 5) LinkedList 리턴한 배열을 출력한다.
    // non-static은 static, non-static 둘다가능
    for (Object obj : arr) {
...
/*
# 중첩 클래스 (모회사 자회사 편입이랑 비슷)
- 어떤 클래스에 같은 식구(멤버)로 만드는 문법이다.
...
- 식구의 종류에는 크게 두가지가 있는데,
 ...
 2) non-static 식구(nested class)
      그 집안 (바깥 클래스)의 static 멤버뿐만 아니라 non-static 멤버도 사용할 수 있다.
      
non-static 멤버?(non-static 자체가 인스턴스를 가리킴)
