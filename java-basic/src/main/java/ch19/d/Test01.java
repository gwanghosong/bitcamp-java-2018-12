// static nested class를 이용하여 상수를 관리할 때 이점
package ch19.d;
...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    // 그 하위필드도 마찬가지로 점(.)을 이용하여 지정할 수 있다.
    // 이런식으로 점(.)을 이용하여 하위 필드를 지정하는 표기법을 OGNL이라 한다.
    //
    // static nested class를 사용하여 더 극단적으로 분류값을 OGNL 표기법으로 사용할 수 있다.
    // 다음과 같이 하나의 이름을 사용하는 대신에 점(.)을 사용하여 하위 프로퍼티를 접근하는 방식은
    // 코드를 읽기 쉽고 유지보수하기 쉽게 도와준다.
    // 이렇게 클래스 이름을 필드처럼 사용할 수 있어 보기가 쉽다.
    p.category = Category.computer.mouse.bluetooth;
    
    // static nested class를 이런식으로 적용한 대표적인 예가
    // 안드로이드 컴파일러가 자동 생성하는 R.java이다.
    // 개선점:
    // 카테고리의 이름이 하나의 긴 변수명을 되어있다.
