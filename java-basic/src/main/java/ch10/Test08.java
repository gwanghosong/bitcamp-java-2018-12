...
class Monitor6 {
  ...
  //
  // 생성자의 호출 절차
  // 인스턴스 생성 > 인스턴스 필드 초기화 문장 시행 > 인스턴스 블록 실행 > 생성자 실행
  //
  // 생성자 호출하는 방법
  // new 클래스명(생성자의 파라미터값, 생성자의 파라미터값, ...);
...
public class Test08 {
  ...
  public static void main(String[] args) {
    ...
    // 1) 생성자의 파라미터 값을 주지않고 인스턴스를 생성할 수는 없다.
    // 인스턴스를 생성한 후 호출될 생성자를 지정해야 한다.
    // 지정하지 않으면 컴파일 오류가 발생한다.
    //new Monitor6; // 컴파일 오류!
    
    // 2)파라미터가 없는 생성자를 호출할 경우 값 없이 지정한다. 괄호를 표시한다.
    new Monitor6();//OK
