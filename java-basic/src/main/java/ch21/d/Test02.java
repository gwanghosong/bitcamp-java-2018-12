...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    // 반드시 예외처리를 해야한다.
    // try ~ catch 로 처리하든, throws 절을 메서드 선언부에 붙이든 둘 중 하나는 해야 한다.
    //
    for(int i = 0; i < list.size(); i++) {
      String name = getName(i);
      // 예외 상황을 검사하기 위해 getName()의 리턴값을 확인한다.
      // null 이면 예외가 발생했다는 것이다.
      if (name == null) {
    ...
    if (result == -1919191919) { // 리턴 값이 -1919191919이기 때문에 오류로 판단한다.
      System.out.println("나누기 오류 입니다."); // 이것이 리턴 값으로 오류를 전달할 때의 문제이다.
    } else {
