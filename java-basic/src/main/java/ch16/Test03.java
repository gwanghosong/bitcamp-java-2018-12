...
public class Test03 {
  ...
  // echo()는 Ok라는 타입의 파라미터(객체주소) 값을 받고, Ok라는 타입의 객체(주소)를 리턴한다.
  // 제네릭을 사용하면 특정 타입을 다루는 메서드를 여러개를 한 개의 메서드로 줄일 수 있다.
  // 이 때 "Ok"를 타입을 가리키는 별명으로 "타입 파라미터"라고 부른다.
...
T - Type이라는 의미로 표현할 수 있어 많이 사용하는 이름이다.
E - Element라는 의미로 목록의 항목을 가리키는 이름으로 사용한다.
K - Key 객체를 가리킬 때 사용한다.
N - Number의 의미로 숫자타입을 가리킬 때 주로 사용한다.
V - Value의 의미로 값의 타입을 가리킬 때 사용한다.
S, U, V 등 - 한 번에 여러 타입을 가리킬 때 두번째, 세번째, 네번째 이름으로 주로 사용한다.

그러나 이름을 짓는것은 개발자 마음이다. 다만, 다른 개발자와의 원활한 소통을 위하여
