...
// 기존의 SqlSessionFactory는 openSession을 요청할 때마다 새로운 SqlSession을 만들어 리턴했는데,
// 같은 스레드에서 openSession을 요청할 때는 SqlSession을 공유하기 위해서 
// ThreadLocal에서 SqlSession객체를 공유하도록 적용한다.
// 이것을 하기 위해 기존의 역할을 대행하는 proxy 패턴을 적용한다.
...
// 원래의 SqlSessionFactry 구현체를 이용해 SqlSession을 만들 것이다.
// 프록시 패턴?
// : 프록시는 반드시 원본 객체와 같은 규칙을 따라야 한다.
// 트랜잭션 지원하기
// : 트랜잭션 시작하고 끝내는 메서드를 추가한다.
//  트랜잭션이 시작할 때 수동 커밋의 SqlSession 객체를 미리 만들어 스레드에 보관한다.
...
public class SqlSessionFactoryProxy implements SqlSessionFactory {
  ...
  // 트랜잭션에 속한 상태가 아닐때는 close()를 수행한다.
  ...
  public SqlSessionFactoryProxy(SqlSessionFactory original) {
    ...
    // 외부에서 요청이 들어오면 이 객체를 사용하여 SqlSession 개체를 만들어 준다.
  ...
  public void prepareForTransaction() {
    // auto commit 이 false인 sqlSession 객체를 준비한다.
  ...
  public SqlSession openSession() {
    ...
    if (sqlSession != null)
      ...
      return original.openSession(true); // mybatis가 기본적으로 false로 설정되어있기때문에
    // 기본은 auto commit이 ture로 설정한다.
  }
