...
public class SqlSessionProxy implements SqlSession{
  ...
  // auto commit이 true/false이냐는 불린값은 
  // sqlSessionProxy객체에 sqlSession을 담을 때 설정한다.
  // 여기서 useTransaction 불린값은 트랜잭션을 사용중이냐 아니냐를 따지는 것이다.
  // 이 메서드를 사용할 때는 보통 트랜잭션을 사용하기 때문에 프록시 객체를 만드는 것이므로
  // 불린 값은 true로 설정될 것이다.
