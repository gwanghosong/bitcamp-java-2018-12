...
//    : Mybatis의 SqlSessionFactory 객체를 생성할 때 어떤 로깅 라이브러리를 사용할지 지정한다. 
//    : MybatisConfig.java 에서 SqlSessionFactory 생성하는 메서드 안에
//      LogFactory.useLog4jLogging(); 코드 추가
//      설정안하면 로그가 출력안됨.
//
// Log4j 설정법
// : CLASSPATH 루트 패키지(src/main/resources/log4j.properties)에 
//  log4j.properties 이름으로 설정 파일을 둔다.
// 1) 출력 등급 설정
...
//    특정 패키지에 전체나 특정 클래스에 대해 등급을 바꿀 때 사용한다.
// : 다음과 같이 등급을 조정할 수 있다.
//    rootLogger = DEBUG
...
// : 등급
//    FATAL: 애플리케이션을 중지할만큼 치명적인 오류일 경우. 예) DBMS 연결 안됨.
//    ERROR: 계속 애플리케이션을 실행해도 되는 오류일 경우. 예) 날짜 형식이 잘못되어 형변환 오류가 발생.
//    WARN: 잠재적인 위험을 안고 있는 경우. 예) 서버와의 소켓 연결을 close() 하지 못했을 때.
//    INFO: 애플리케이션의 주요 실행 정보. 예) 어떤 IP의 클라이언트가 접속 정보. 연결 종료. 관리자가 볼 정보.
//    DEBUG: 애플리케이션의 내부 실행 상태를 추적해 볼 수 있는 정보. 개발자가 볼 정보.
...
//      INFO보다 큰 레벨(WARN, ERROR, FATAL)의 로그도 출력한다. 
//
// 2) 출력 대상 지정
// : 어디로 출력할 것인지 지정한다.
// : log4j.rootLogger=출력등급, 담당자이름
//    log4j.appender.담당자이름=담당자가 출력하기 위해 사용할 도구
//      예)
...
//
// 4) 출력 형식의 패턴 설정하기
// : PatternLayout을 사용할 때 설정하는 방법.
// : 문법
//    %p          - 출력 등급. 예) FATAL, ERROR, WARN, INFO, DEBUG, TRACE
//    %자릿수p      - 출력 문자열 자릿수를 지정할 수 있다.
