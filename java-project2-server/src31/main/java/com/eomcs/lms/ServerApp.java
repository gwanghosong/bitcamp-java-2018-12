...
//    : 받을 버전들어가서 core zip버전 다운
//    : ~/server/$톰캣홈
//    : 실행하고 자바 jvm 버전 확인. 다르면 리눅스 환경변수에서 $JAVA_HOME 버전 교체
// 2) 자바 웹 프로젝트로 전환
//    : build.gradle 변경
//      - 'eclipse-wtp' 플러그인과 'war' 플러그인 추가한다.
//      - 'servlet-api' 라이브러리 추가 (Tomcat 서버 버전에 따라 documentation 확인하고 버전 맞춰서 설치)
//    : '$ gradle eclipse' 실행
//    : 프로젝트 리프레시
//      - 이클립스 웹 프로젝트 관련 디렉토리와 파일이 추가되었다.
// 3) ServerApp 변경
//    : 'Servlet'이라는 규칙에 따라 클래스를 변경한다.
//    : 이 규칙에 따라 작성해야만 톰캣 서버가 호출 해 줄 것이다.
...
//    : OS에 설치한 톰캣 서버 경로를 등록
// 2) 웹 애플리케이션 테스트를 할 때 사용할 톰캣 배치 폴더 생성
//    : 'Servers' 뷰 선택(Window/show view)
//    : New/Server 메뉴 클릭
//    : 이클립스에 등록된 서버중에서 테스트 환경을 구축할 서버 선택
//    : 서버 이름 지정한 후 확인 클릭
...
//      또는 Servers폴더 .properties에서 경로클릭후 찾기
//      또는 즐겨찾기 해놓자.
// 3) 웹 애플리케이션을 테스트 용으로 만든 배치 폴더에 배포한다.
//    : 'gradle build'를 실행할 필요가 없다.
//    : 'Servers' 뷰 선택
//    : 테스트 서버에 대해 컨텍스트 메뉴(우클릭) 출력
...
//    : Start/Stop 실행
//
// cf)  테스트 할 때 정상적으로 클래스 실행 안될 시 이클립스 오류로 인해서 안될 수도 있으니
//      여기 경로안에 정상적으로 클래스파일이 생성되는지 확인할때도 여기서 확인
//      그 다음으로 네비게이터에서 bin 폴더 아래에 클래스 생성됬는지 확인
//      그 다음으로 project-Build Automatically 확인
//      그래도 안되면 workspace 폴더를 바꿈.(폴더 이름을 바꾸면 이클립스가 새로 생성함.)
//      이런 경우 preferences를 다시 설정해야함.
// cf) tomcat 서버 log파일 실시간 확인
// : $ tail -f catailna.out(파일명)
//
...
public class ServerApp implements Servlet {
  ...
  public void service(
    ...
    response.setContentType("text/html;charset=UTF-8"); // 반드시 출력스트림 얻기 전에 실행
