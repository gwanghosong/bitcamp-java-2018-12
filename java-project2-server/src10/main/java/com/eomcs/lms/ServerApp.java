// 10단계: 멀티스레드 상황에서 DB 커넥션을 공유할 때 문제점과 그 해결책 I
// 
// [멀티스레드에서 DB커넥션 공유할 때의 문제점 확인하기]
// 1) 클라이언트1
...
//       rollback 할 수 없다.
//        - 이것이 멀티 스레드에서 DB커넥션을 공유할 때의 문제점이다.
//
// [해결책 I]
...
//      => 이 때 첨부파일 두 개를 입력한 후, 세번째 입력할 때 예외가 발생하도록
//           긴 이름의 파일명을 입력한다.
//       => 예외가 발생했다는 메시지를 받을 것이다.
// 6) /photoboard/list 실행한다.
//        => 예외가 발생하기 전 입력했던 데이터가 그대로 DB에 보관된 것을 확인할 수 있다.
