...
public class AppConfig {
  ...
  public DataSource dataSource() {
    ...
    // Spring IoC 컨테이너에서 주입받은 env 객체를 사용하여 이 파일의 값을 꺼내면 된다.
  ...
  // Connection의 commit/rollback을 관리할 트랜잭션 관리자를 생성하는 메서드
  ...
  //  따라서 트랜잭션 관리자를 만드려면 DataSource객체가 있어야 한다.
  ...
  public SqlSessionFactory sqlSessionFactory(
    ...
    // MyBatis에서 SqlSessionFactory를 생성할 때 사용하라고 제공하는 객체이다.(이전의 Builder객체) 
    SqlSessionFactoryBean factoryBean = new SqlSessionFactoryBean();
    
    // SqlSessionFactory 객체를 생성하는데 필요한 객체를 주입한다.
    factoryBean.setDataSource(dataSource);
    
    // 도메인 클래스의 별명을 자동으로 생성하도록 
    // 도메인 클래스가 들어있는 패키지를 지정한다.
  ...
  /*
  // DAO 구현체 자동 생성하기 방법1:
  // : MyBatis에서 Spring과 함께 사용하라고 제공해주는 SqlSessionTemplate를 사용한다.
  // : @MapperScan 애노테이션을 사용하면 더 간단히 DAO 구현체를 자동생성할 수 있다. 
  @Bean
