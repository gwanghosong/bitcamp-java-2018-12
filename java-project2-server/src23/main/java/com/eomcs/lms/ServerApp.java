...
//  이클립스 설정 파일을 갱신한다.
//  이클립스 IDE에서 프로젝트를 리프레시한다.
// 2) 의존 라이브러리 준비
// : DataSource 구현체인 apache의 commons-dbcp2 라이브러리 추가한다.
//  Spring의 jdbc관련 라이브러리 추가 - 트랜잭션 관련 라이브러리도 자동으로 추가된다.
// 3) AppConfig 변경
// : MyBatis-config.xml 삭제한다.
//  SqlSessionFactoryProxy, SqlSessionProxy, TransactionManager 삭제(Spring으로 교체함.)
//  DaoFactory 삭제한다.
//  MyBatis 관련 객체를 생성한다.
