  ...
  private void findClasses(File dir, String packageName) throws Exception {
    ...
    File[] files = dir.listFiles((File pathname) -> {
      ...
      if (pathname.getName().endsWith(".class") && // 자바 클래스 파일
          !pathname.getName().contains("$")) // 중첩클래스가 아닌 경우
        // 중첩클래스인 경우 $표시로 그 패키지안에 class파일로 표현됨.
        return true;
    ...
    for (File f : files) {
      ...
      if (f.isFile()) {
        ...
        //  예) com.eomcs.lms(패키지명) + . + ServerApp(파일명) = com.eomcs.lms.Server
...
//        System.out.println(f.getName());
  ...
  private void createComponent() throws Exception {
    ...
    for (Class<?> clazz : classes) {
      ...
      if (obj != null) { // 제대로 생성했으면 beanContainer에 저장한다.
        // 빈컨테이너에 객체를 저장할 때 key값은 Component 애노테이션의 value()값으로 사용한다.
        // 만약 value가 빈 문자이라면 클래스 이름을 사용한다.
        addBean(
  ...
  private Object[] getParameterValues(Class<?>[] paramTypes) {
    ...
    if(values.size() == paramTypes.length)
      // 파라미터 타입 목록에 지정된 객체를 모두 찾았으면 배열로 리턴한다. 
  ...
  private Object findBean(Class<?> type) {
    ...
    // : 먼저 beanContainer에서 인스턴스 목록을 꺼낸다.
  ...
  private Object callFactoryMethod(
      ...
      List<Method> factoryMethods) throws Exception {
    ...
    for (int i = 0; i < paramTypes.length; i++) {
      // 2) 빈컨테이너에서 파라미터 타입에 해당하는 값을 꺼낸다.
      ...
      // 3) 만약 빈컨테이너에 파라미터 타입에 해당하는 값이 없다면 
      ...
      if (otherFactoryMethod == null) {
        ...
        // beanContainer와 factoryMethods 중 어느곳에서도 파라미터를 만들어줄 메서드가 없기 때문에
        // 이 메서드를 호출할 수 없다. 호출을 포기한다.
        return null;
      }

      // 파라미터 값을 만들어 줄 다른 팩토리 메서드를 찾았다면, 그 메서드를 호출한다.
      // : 재귀 호출을 사용한다.
      paramValue = callFactoryMethod(
          obj, 
          otherFactoryMethod, 
          factoryMethods);
      if (paramValue == null) {
        // 파라미터 값을 리턴해주는 팩토리 메서드를 찾긴 했지만, 
        // 그 메서드를 호출하기 위해 또 다른 파라미터 값이 필요했는데
        // 그 파라미터 값이 없어서 그 메서드 호출에 실패했다면,
        // 파라미터 값을 준비하지 못했기 때문에 이때도 factoryMethod 호출을 포기한다.
        return null;
      }
    ...
    // 5) 준비한 파라미터 값들을 가지고 팩토리 메서드를 호출한다
    //  그리고 팩토리 메서드가 생성한 객체를 빈 컨테이너에 보관한다.
    Object bean = factoryMethod.invoke(obj, paramValues);
    
    // 팩토리메서드가 생성한 객체를 저장할 때,
    // 애노테이션에 빈의 이름이 지정되어있다면 그 이름을 사용하고
    // 없다면 메서드 이름을 사용한다.
